...
export default function ApplicationShell({ children }) {
  ...
  return (
    ...
    <div className="flex h-screen gap-3 overflow-hidden bg-gray-50 p-3 dark:bg-gray-950">
      ...
      {/* ── Skip link — visually hidden, shown on focus for keyboard users ── */}
      <a
        href="#main-content"
      ...
      <div className="flex flex-1 min-w-0 flex-col gap-3">
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 rounded-2xl bg-white px-4 shadow-sm dark:bg-gray-800">
          ...
          {/* Search bar — navigates to /audits?q=... on submit */}
          <form
            role="search"
            className="relative hidden sm:flex items-center"
            onSubmit={e => {
              e.preventDefault()
              const q = e.currentTarget.elements['global-search'].value.trim()
              if (q) navigate(`/audits?q=${encodeURIComponent(q)}`)
            }}
          >
            <Search
        ...
        <main id="main-content" tabIndex={-1} className="flex-1 min-h-0 overflow-auto rounded-2xl focus:outline-none">
          {children}
        </main>
      </div>
