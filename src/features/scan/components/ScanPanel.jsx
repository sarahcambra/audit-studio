...
export default function ScanPanel({ audit, auditId, userId, onScanComplete }) {
  ...
  // Columns for scan history DataTable
  const historyColumns = [
    { key: 'name', header: 'Name', width: 'min-w-48', render: (job) => (
      <span className="font-medium text-gray-900 dark:text-white">{job.scanName}</span>
    )},
    { key: 'type', header: 'Type', width: 'w-24', render: (job) => (
      <span className="text-sm capitalize text-gray-500 dark:text-gray-400">{job.scanType}</span>
    )},
    { key: 'result', header: 'Result', width: 'w-40', render: (job) => (
    ...
    { key: 'actions', header: '', width: 'w-24', render: (job) => (
      <button
  ...
  return (
    <div className="grid grid-cols-1 gap-4 px-4 pt-6">
      <PageHeader title="Run Scans" subtitle="Select scan type and configure settings" />
      ...
      {/* ── Scan errors ─────────────────────────────────────────────────── */}
      {errorJobs.length > 0 && (
        <div className="overflow-hidden rounded-lg bg-red-50 shadow-sm dark:bg-red-950 border border-red-200 dark:border-red-800">
          <div className="flex items-center gap-3 p-5 border-b border-red-200 dark:border-red-800">
          ...
          <div className="divide-y divide-red-200 dark:divide-red-800 px-5 pb-5">
            {errorJobs.map(job => (
