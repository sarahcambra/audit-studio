...
import Step3PreTest from './steps/Step3PreTest'
...
export default function NewAuditWizard({ onClose }) {
  ...
  const handleNext = () => {
    ...
    if (currentStep < 5) {
      // Keep showValidationErrors true so errors re-appear if user returns to this step
      setShowValidationErrors(true)
  ...
  const handleBack = () => {
    if (currentStep > 1) {
      ...
      // and clear naturally when the user fixes the fields.
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (error) {
                    ...
                    toast.error(`Could not save audit — ${detail}.`)
                    return
                  }
                  if (form.websiteUrl && data?.id) {
                    const normUrl = normaliseUrl(form.websiteUrl)
                    fetch(`/api/favicon?url=${encodeURIComponent(normUrl)}`)
                      .then(r => r.ok ? r.json() : null)
                      .then(json => {
                        if (json?.faviconUrl) updateAuditFavicon(data.id, json.faviconUrl)
                        else console.warn('[favicon] No faviconUrl returned for', normUrl)
                      })
                      .catch(err => console.warn('[favicon] fetch failed:', err.message))
                  }
                  setShowToast(true)
                  setTimeout(() => {
