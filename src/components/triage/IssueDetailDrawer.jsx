...
export default function IssueDetailDrawer({
  ...
  return (
    <>
      ...
      >
        {/* ── STICKY HEADER ── */}
        <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-default px-5 py-3 flex items-start gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h2 id="drawer-issue-title" className="text-sm font-semibold text-heading leading-snug">
              {enrichment?.auditorTitle ?? item.rule_id}
            </h2>
            {item.page_name && (
              <p className="text-xs text-body-subtle mt-0.5">{item.page_name}</p>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* 1 — SCREENSHOT */}
          <div>
            <p className="text-xs font-semibold text-body-subtle uppercase tracking-wide mb-2">
              Screenshot / Evidence
            </p>
            {item.screenshot_url ? (
              <button
                onClick={() => setLightboxOpen(true)}
                className="w-full rounded-xl overflow-hidden border border-default bg-neutral-tertiary hover:ring-2 hover:ring-primary-300 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300"
              ...
              <div className="w-full rounded-xl border-2 border-dashed border-gray-300 bg-neutral-tertiary flex flex-col items-center justify-center py-8 gap-2">
                <ImageOff className="h-8 w-8 text-body-subtle" />
                <p className="text-xs text-body-subtle text-center">
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <label className={`cursor-pointer ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                />
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-primary-700 hover:text-primary-800 bg-primary-50 hover:bg-primary-100 border border-primary-200 rounded-lg px-2.5 py-1.5 transition-colors cursor-pointer">
                  <Upload className="h-3.5 w-3.5" />
                  {isUploading ? 'Uploading…' : 'Add image / video'}
                </span>
              </label>
              <p className="text-xs text-body-subtle">Extra evidence for report</p>
              {uploadError && (
                <p className="w-full text-xs text-fg-danger mt-1">{uploadError}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <p className="text-xs font-semibold text-body-subtle uppercase tracking-wide">
              Code Evidence
            </p>
            ...
            {item.element_snippet && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-medium text-body-subtle">Element</span>
                  <Button
                    size="xs"
                    color="ghost"
                    theme={customTheme.button}
            ...
            {item.html_snippet && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-medium text-body-subtle">HTML Snippet</span>
                  <Button
                    size="xs"
                    color="ghost"
                    theme={customTheme.button}
                ...
                {item.selector && (
                  <p
                    className="text-xs font-mono text-body-subtle mt-1.5 truncate"
                    title={item.selector}
                  >
                    {item.selector}
                  </p>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-medium text-body-subtle">Additional code fragment</span>
            ...
            <Accordion collapseAll={false}>
              <AccordionPanel>
                ...
                <AccordionContent className="space-y-4">
                  {/* Editable clientFix */}
                  <div>
                    <label className="text-xs font-medium text-body-subtle mb-1 block">
                      Fix description
                    </label>
                    <textarea
                      value={val('clientFix') ?? ''}
                      ...
                      rows={3}
                      className="w-full text-sm text-body bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-primary-300 resize-y"
                    />
                  </div>

                  {/* fixDifficulty selector */}
                  <div className="flex items-center gap-2">
                    <label className="text-xs font-medium text-body-subtle">Complexity</label>
                    <select
                      ...
                      className="text-xs border border-gray-300 rounded-lg px-2 py-1 bg-white dark:bg-gray-800 text-body focus:ring-2 focus:ring-primary-300"
                    >
                      <option value="">—</option>
            ...
            <AccordionPanel>
              ...
              <AccordionContent className="space-y-4">
                ...
                {item.wcag_sc && WCAG_REFERENCES[item.wcag_sc] && (
                  <div>
                    <p className="text-xs font-medium text-body-subtle mb-1.5">
                      Success Criterion
                    </p>
                    <a
                      href={WCAG_REFERENCES[item.wcag_sc].url}
                ...
                {enrichment.wcagTechniques?.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-body-subtle mb-1.5">W3C Techniques</p>
                ...
                {enrichment.wcagFailures?.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-body-subtle mb-1.5">Common Failures</p>
                    <ul className="space-y-1.5">
        ...
        {/* ── STICKY FOOTER ── */}
        <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-default px-5 py-3 flex items-center gap-2 justify-between">
          ...
          <span className="text-xs text-body-subtle shrink-0">
